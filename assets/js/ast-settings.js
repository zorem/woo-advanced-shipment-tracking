...
(function () {
	...
	ready(function () {
		...
		root.addEventListener('click', function (e) {
			var editBtn = e.target.closest('.ast-set-carrier-edit');
			if (!editBtn) { return; }
			var card = editBtn.closest('.ast-set-carrier-card');
			var modal = openModal('edit');
			if (!modal) { return; }

			// Preview from the card (instant).
			if (card) {
				var name = ((card.querySelector('.ast-set-carrier-name') || {}).textContent || '').trim();
				var country = ((card.querySelector('.ast-set-carrier-country') || {}).textContent || '').trim();
				var logo = card.querySelector('.ast-set-carrier-logo');
				Array.prototype.forEach.call(modal.querySelectorAll('.ast-edit-carrier-name, .ast-edit-carrier-name-2'), function (el) { el.textContent = name; });
				var cEl = modal.querySelector('.ast-edit-carrier-country'); if (cEl) { cEl.textContent = country; }
				var lEl = modal.querySelector('.ast-edit-carrier-logo'); if (lEl && logo) { lEl.innerHTML = logo.innerHTML; }
			}

			var pid = editBtn.getAttribute('data-pid') || '';
			modal.querySelector('.ast-edit-provider-id').value = pid;
			modal.querySelector('.ast-edit-provider-type').value = editBtn.getAttribute('data-type') || '';

			// Reset fields, then load real values.
			setVal(modal, '.ast-edit-display-name', '');
			setVal(modal, '.ast-edit-url', '');
			setVal(modal, '.ast-edit-provider-name', '');
			setVal(modal, '.ast-edit-shipping-country', '');
			setVal(modal, '.ast-carrier-thumb-url', '');
			setVal(modal, '.ast-carrier-thumb-id', '');
			var def = modal.querySelector('input[name="make_provider_default"]'); if (def) { def.checked = false; }
			fillApiNames(modal, ['']);
			syncLogoRemove(modal.querySelector('.zui-upload'));

			// Show loader overlay while AJAX populates the form (same pattern as Edit Integration).
			var form = modal.querySelector('#ast-edit-carrier-form');
			var loader = modal.querySelector('#ast-edit-carrier-loader');
			if (form) { form.setAttribute('aria-busy', 'true'); }
			if (loader) { loader.removeAttribute('hidden'); }
			function hideCarrierLoader() {
			...
			post({ action: 'ast_carrier_details', security: NONCE, provider_id: pid })
				...
				.then(function (res) {
					...
					var d = res.data;
					setVal(modal, '.ast-edit-display-name', d.display_name || '');
					setVal(modal, '.ast-edit-url', d.tracking_url || '');
					setVal(modal, '.ast-edit-provider-name', d.provider_name || '');
					setVal(modal, '.ast-edit-shipping-country', d.shipping_country || '');
					setVal(modal, '.ast-carrier-thumb-url', d.thumb_url || '');
					setVal(modal, '.ast-carrier-thumb-id', d.thumb_id || '');
					if (def) { def.checked = !!d.is_default; }
					fillApiNames(modal, (d.api_names && d.api_names.length) ? d.api_names : ['']);
					syncLogoRemove(modal.querySelector('.zui-upload'));
				})
				.catch(function () { hideCarrierLoader(); });
		});
		...
		function fillApiNames(modal, names) {
			...
			if (!wrap) { return; }
			var rows = wrap.querySelectorAll('.ast-set-apiname-row');
			for (var i = rows.length - 1; i >= 1; i--) { rows[i].remove(); }
			var first = wrap.querySelector('.ast-set-apiname-row input');
			if (first) { first.value = names[0] || ''; }
			for (var j = 1; j < names.length; j++) {
				var row = document.createElement('div');
				row.className = 'ast-set-apiname-row';
				row.innerHTML = '<input type="text" name="api_provider_name[]" class="zui-field__input" placeholder="API Name">' +
		...
		root.addEventListener('click', function (e) {
			var addBtn = e.target.closest('.ast-set-apiname-add');
			if (addBtn) {
			...
			var rm = e.target.closest('.ast-set-apiname-remove');
			if (rm) { var r = rm.closest('.ast-set-apiname-row'); if (r) { r.remove(); } }
		});

		/* ---------- Add / Edit form submit ---------- */
		var addForm = document.getElementById('ast-add-carrier-form');
		if (addForm) {
			var addLoader = document.getElementById('ast-add-carrier-loader');
			var addError  = document.getElementById('ast-add-carrier-error');
			function showAddLoader() { if (addLoader) { addLoader.removeAttribute('hidden'); } addForm.setAttribute('aria-busy', 'true'); }
			function hideAddLoader() { if (addLoader) { addLoader.setAttribute('hidden', ''); } addForm.setAttribute('aria-busy', 'false'); }
			function showAddError(msg) { if (addError) { addError.textContent = msg; addError.removeAttribute('hidden'); } }
			function clearAddError()   { if (addError) { addError.setAttribute('hidden', ''); addError.textContent = ''; } }

			// Auto-clear the duplicate error as soon as the user changes name or country —
			// no point yelling at them while they're already fixing it.
			var nameInput    = addForm.querySelector('[name="shipping_provider"]');
			var countrySelect = addForm.querySelector('[name="shipping_country"]');
			if (nameInput)    { nameInput.addEventListener('input', clearAddError); }
			if (countrySelect) { countrySelect.addEventListener('change', clearAddError); }
			...
			addForm.addEventListener('submit', function (e) {
				...
				postForm(addForm)
					...
					.then(function (res) {
						// Backend returns wp_send_json_error on duplicate (same provider_name + shipping_country).
						if (res && res.success === false) {
							if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = origText; }
							hideAddLoader();
							showAddError((res.data && res.data.message) ? res.data.message : 'Could not add carrier.');
							return;
						}
						...
						setTimeout(function () {
							...
							addForm.reset();
							refreshGrid();
							if (window.jQuery && jQuery(document).ast_snackbar) { jQuery(document).ast_snackbar('Custom carrier added'); }
						}, 800);
					})
					.catch(function () {
						if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = origText; }
		...
		if (editForm) {
			// Helpers to show/hide the dialog-level loader during save / reset AJAX.
			function showEditLoader() {
				var loader = document.getElementById('ast-edit-carrier-loader');
				if (loader) { loader.removeAttribute('hidden'); }
	...
	function initCsv() {
		...
		if (ftpForm) {
			ftpForm.addEventListener('submit', function (e) {
				e.preventDefault();
				var ajaxurl = window.ajaxurl || (window.location.origin + '/wp-admin/admin-ajax.php');
				var btn = ftpForm.querySelector('.ast-csv-ftp-save');
...
(function () {
	...
	var ATTRS = [
		...
		{ key: 'sku', label: 'sku', desc: 'Product SKU (for per-product tracking)', required: false, kws: ['sku', 'product_sku', 'product'] },
		{ key: 'qty', label: 'qty', desc: 'Quantity shipped for this SKU', required: false, kws: ['qty', 'quantity', 'count'] }
	];
	...
	function initBp() {
		...
		var steps = root.querySelectorAll('.ast-set-bp-step');
		var stepLis = root.querySelectorAll('#ast-bp-steps li');

		function currentMode() { return root.dataset.bpmode === 'tpi' ? 'tpi' : 'simple'; }
		function visibleAttrs() {
			if (currentMode() === 'tpi') { return ATTRS; }
			return ATTRS.filter(function (a) { return a.key !== 'sku' && a.key !== 'qty'; });
		}
		var SAMPLES = {
			simple:
		...
		function refreshPlaceholder() {
			...
			if (currentMode() === 'tpi') {
				ta.setAttribute('placeholder',
					'order_id,tracking_provider,tracking_number,date_shipped,status_shipped,shipping_note,sku,qty\n' +
					'#18402,UPS,1Z999AA10123456784,02-06-2026,Completed,Fragile packages,Woo-tshirt-logo,2\n' +
					'#18401,FedEx,48123098234,02-06-2026,Completed,Handled carefully,woo-polo,1');
			} else {
				ta.setAttribute('placeholder',
					'order_id,tracking_provider,tracking_number,date_shipped,status_shipped,shipping_note\n' +
					'#18402,UPS,1Z999AA10123456784,02-06-2026,Completed,Fragile packages\n' +
					'#18401,FedEx,48123098234,02-06-2026,Completed,Handled carefully');
			}
		}
		...
		function autoMatch(attr, headers, hasHeaders, posIndex) {
			// Pass 1: prefer an EXACT keyword match. Without this, the partial-match
			// pass below maps `tracking_number` to the `tracking_provider` header
			// because 'tracking' (a kw of tracking_number) is a substring of
			// 'tracking_provider' AND that header appears first.
			for (var i = 0; i < headers.length; i++) {
				var h = headers[i].toLowerCase();
				for (var k = 0; k < attr.kws.length; k++) {
					if (h === attr.kws[k]) { return headers[i]; }
				}
			}
			// Pass 2: substring fallback (kept for forgiving header names like
			// "Order Number", "Tracking #").
			for (var i = 0; i < headers.length; i++) {
				var h = headers[i].toLowerCase();
				for (var k = 0; k < attr.kws.length; k++) {
					if (h.indexOf(attr.kws[k]) !== -1) { return headers[i]; }
				}
			}
		...
		function buildMap(headers, hasHeaders) {
			...
			if (!body) { return; }
			body.innerHTML = '';
			visibleAttrs().forEach(function (attr, pos) {
				...
				row.innerHTML =
					...
					'<div class="ast-set-bp-maprow__sel"><div class="zui-select-wrap"><select class="zui-select" data-bpkey="' + attr.key + '">' + opts + '</select><span class="zui-select-chevron"><svg class="zui-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="6 9 12 15 18 9"/></svg></span></div>' + prev + '</div>';
		...
		function applyRow(row, map, replace) {
			...
			// Strip a leading "#" that customers commonly carry over from Excel
			// ("#18402" → "18402") so the order lookup doesn't fail on the prefix.
			var orderIdRaw = String( cellValue(row, map.order_id) || '' ).trim();
			body.append('order_id', orderIdRaw.replace(/^#+/, ''));
			...
			if (currentMode() === 'tpi') {
				body.append('sku', cellValue(row, map.sku));
				body.append('qty', cellValue(row, map.qty));
			}
			body.append('replace_tracking_info', replace ? '1' : '');
			body.append('date_format_for_csv_import', (root.dataset.dateformat || 'd-m-Y'));
			// Distinguish Bulk Paste from CSV Import in the order note (both share this AJAX action).
			body.append('source', 'bulk_paste');
		...
		function run() {
			...
			var rows = state.rows;
			var log = $('#ast-bp-log');
			var bar = root.querySelector('.ast-bp-bar');
			var num = root.querySelector('.ast-bp-progress-num');
		...
		function finish(ok, fail) {
			var okEl = root.querySelector('.ast-bp-stat-ok'); if (okEl) { okEl.textContent = ok; }
			var failEl = root.querySelector('.ast-bp-stat-fail'); if (failEl) { failEl.textContent = fail; }
		...
		if (sampleBtn) {
			sampleBtn.addEventListener('click', function () {
				ta.value = SAMPLES[currentMode()];
				$('#ast-bp-headers').checked = true;
				refreshRowCount();
			});
		}
		var modeTabs = root.querySelectorAll('.zui-segmented__item[data-bptab]');
		Array.prototype.forEach.call(modeTabs, function (btn) {
			btn.addEventListener('click', function () {
				root.dataset.bpmode = btn.getAttribute('data-bptab');
				Array.prototype.forEach.call(modeTabs, function (b) {
					var on = b === btn;
					b.classList.toggle('is-active', on);
					b.setAttribute('aria-selected', on ? 'true' : 'false');
				});
				refreshPlaceholder();
				// If textarea currently shows a known sample (not customized), swap to the
				// new mode's sample so users see the expected columns live.
				if (ta && (ta.value === SAMPLES.simple || ta.value === SAMPLES.tpi)) {
					ta.value = SAMPLES[currentMode()];
					refreshRowCount();
				}
				// If user is currently on map step, rebuild it for the new mode.
				if (state.headers.length) {
					buildMap(state.headers, $('#ast-bp-headers').checked);
				}
			});
		});
